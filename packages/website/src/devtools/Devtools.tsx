...
import { ArrowRight, Bot, Bug, Github, Keyboard, Map as MapIcon, Package, Rocket, ShieldCheck, Sparkles, Wrench } from 'lucide-react';
...
const PACKAGES = [
  {
    ...
    install: 'npm install -g @nayan-ui/cli\n\n# or, without installing\nnpx @nayan-ui/cli [command]',
...
nayan-ui validate robots -i ./robots.txt`,
    features: [
      { icon: Rocket, title: 'Project scaffolding', body: 'Create projects from the Expo, Games, Next.js and Vite templates.' },
      { icon: MapIcon, title: 'Sitemaps', body: 'Crawl a website to generate an XML sitemap, or validate an existing one.' },
      { icon: Bot, title: 'Robots.txt', body: 'Create and validate robots.txt files for crawler control.' },
      { icon: Keyboard, title: 'Interactive or not', body: 'Prompts when you want them, flags when you are scripting CI.' }
    ]
  },
  {
    name: '@nayan-ui/ai',
...
nayan-ai scan <repo-url> -t ghp_xxx --llm claude`,
    features: [
      { icon: Bug, title: 'AI code review', body: 'Deep pull-request analysis for bugs, security, performance and test coverage.' },
      { icon: ShieldCheck, title: 'Vulnerability scanning', body: 'Package vulnerabilities across npm, Python, Go, Rust and more.' },
      { icon: Wrench, title: 'Auto-fix and PRs', body: 'Fix what it finds and open a pull request describing the change.' },
      { icon: Bot, title: 'Two providers', body: 'Codex by default, or Claude Code with one flag.' }
    ]
  }
];
  ...
  return (
    <Sidebar title={component?.title || 'Devtools'}>
      <DocsIntro
        ...
        actions={
          <>
            <Link href="https://www.npmjs.com/package/@nayan-ui/cli" target="_blank" rel="noopener noreferrer" className={BUTTON_SMALL}>
              <Package aria-hidden className="mr-2 h-4 w-4" />
              @nayan-ui/cli
            </Link>
            <Link href="https://www.npmjs.com/package/@nayan-ui/ai" target="_blank" rel="noopener noreferrer" className={BUTTON_SMALL}>
              <Package aria-hidden className="mr-2 h-4 w-4" />
              @nayan-ui/ai
            </Link>
            <Link href="https://github.com/ursnj/nayan-ui" target="_blank" rel="noopener noreferrer" className={BUTTON_SMALL}>
            ...
            return (
              <Link key={tool.href} href={tool.href} className={`${CARD_INTERACTIVE} group p-5`}>
                <div className="mb-3 flex items-center gap-3">
                  ...
                  <ArrowRight aria-hidden className="ml-auto h-4 w-4 shrink-0 text-muted transition-transform group-hover:translate-x-0.5" />
                </div>
        ...
        <SubHeader key={pkg.name} title={pkg.name} description={pkg.lead}>
          <div className="grid min-w-0 gap-5 lg:grid-cols-2">
            <div className="min-w-0 space-y-4">
              <div>
                <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted">Install</h4>
                <Code code={pkg.install} language="bash" filename="terminal" />
              </div>
              <div>
                <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted">Commands</h4>
                <Code code={pkg.commands} language="bash" filename="terminal" />
              </div>
            </div>

            <div className={`${CARD} p-5`}>
              <h4 className="mb-4 text-xs font-semibold uppercase tracking-wider text-muted">What it does</h4>
              <ul className="space-y-4">
                {pkg.features.map(feature => {
                  const Icon = feature.icon;
                  return (
                    <li key={feature.title} className="flex items-start gap-3">
                      <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border ${ACCENT_SOFT}`}>
                        <Icon className="h-4 w-4" />
                      </span>
                      <span>
                        <span className="block text-sm font-semibold text-foreground">{feature.title}</span>
                        <span className="mt-0.5 block text-sm leading-relaxed text-muted">{feature.body}</span>
                      </span>
                    </li>
                  );
                })}
