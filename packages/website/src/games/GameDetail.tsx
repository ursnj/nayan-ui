          ...
          <div className="grid items-center gap-8 md:grid-cols-2">
            <div>
              <div className="mb-4 inline-flex items-center gap-2">
                <span
                  className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${categoryColors[game.category]}`}
                >
                  {game.category}
                </span>
                <span className="text-xs font-medium bg-default/50 text-muted px-2.5 py-1 rounded-full">
                  {game.difficulty}
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {game.tags.map((tag, idx) => (
                  <span
                    key={idx}
                    className="text-[11px] font-medium bg-default/50 text-muted px-2 py-0.5 rounded-md"
                  >
                    #{tag}
                  </span>
                ))}
            ...
            <div className="relative mx-auto max-w-xs">
              <div className="relative aspect-square overflow-hidden rounded-2xl shadow-lg">
                <img
                  ...
                  alt={`${game.name} - ${game.category} Game`}
      ...
      <section className="mb-8">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {game.tips.map((tip, index) => (
            <div
              key={index}
              className="rounded-xl border border-default bg-surface p-4 transition-shadow hover:shadow-md"
            >
