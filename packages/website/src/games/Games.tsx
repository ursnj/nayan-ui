...
import { Gamepad2, Github, Package, Settings2, Smartphone, Sparkles, Volume2, Zap } from 'lucide-react';
import Link from 'next/link';
import { usePathname } from 'next/navigation';
import { CheckList, DocsIntro } from '@/design/Primitives';
import { ACCENT_SOFT, BUTTON_SMALL, CARD, CARD_INTERACTIVE, GRID_GAP, H3, PILL } from '@/design/system';
...
const CAPABILITIES = [
  ...
  },
  {
    ...
    items: ['iOS, Android and Web', 'Consistent performance across platforms', 'Responsive to every screen size', 'Notch-aware offset handling']
  },
  {
    icon: Volume2,
    title: 'Audio and haptics',
    items: ['Sound effects and speech synthesis', 'Haptic feedback', 'Configurable audio settings', 'Per-game sound customisation']
  },
  {
    icon: Settings2,
    title: 'Customisable',
    items: ['Multiple difficulty levels', 'Adjustable game parameters', 'Persistent settings storage', 'One settings interface for all of them']
  },
  {
    icon: Sparkles,
    title: 'Game features',
    items: ['Physics simulation and collisions', 'Particle systems and visual effects', 'Procedural mazes and obstacles', 'Scores and achievements']
  },
  {
    icon: Gamepad2,
    title: 'Developer friendly',
    items: ['TypeScript throughout', 'Shared GameControlButton and ScoreBoard', 'One consistent API across every game', 'Documented per game']
  }
];
  ...
  return (
    <Sidebar title={component?.title || 'Games'}>
      <DocsIntro
        lead={
          <>
            {GAME_COUNT} ready-made games for React Native projects, built on Skia, Reanimated, Gesture Handler and TypeScript for 60fps gameplay
            across iOS, Android and the web. Drop one into a screen, or read how it works and build your own.
          </>
        }
        actions={
          <>
            <Link href="https://www.npmjs.com/package/@nayan-ui/games" target="_blank" rel="noopener noreferrer" className={BUTTON_SMALL}>
      ...
      <SubHeader title="What you get" description="Every game in the package is built on the same foundations.">
      ...
      <SubHeader
        ...
        action={
          <div className="hidden flex-wrap gap-1.5 sm:flex">
            {categories.map(category => (
              <span key={category} className={`text-xs font-semibold ${PILL} ${categoryColors[category] ?? ''}`}>
                {category}
              </span>
            ))}
          </div>
        ...
        <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 ${GRID_GAP}`}>
          {GAMES_LIST.map(game => (
            <Link key={game.id} href={`/games/${game.id}`} className={`${CARD_INTERACTIVE} group overflow-hidden`}>
              <div className="relative overflow-hidden">
                <img
                  ...
                  loading="lazy"
                />
                <span className={`absolute right-3 top-3 rounded-full border px-2.5 py-1 text-xs font-semibold ${categoryColors[game.category]}`}>
                  {game.category}
                </span>
              ...
              <div className="p-4">
                ...
                <ul className="flex flex-wrap gap-1.5">
                  {game.tags.map(tag => (
                    <li key={tag} className="rounded-md bg-default/50 px-2 py-0.5 text-[11px] font-medium text-muted">
                      #{tag}
                    </li>
                  ))}
                </ul>
