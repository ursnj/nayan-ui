...
export const NSearchField = React.memo<NSearchFieldProps>(
  ({ placeholder = 'Search...', hideSearchIcon = false, hideClearButton = false, className, groupClassName, inputClassName, ...props }) => {
    return (
      <SearchField className={cn('mb-3', className)} {...props}>
        <SearchField.Group className={cn(groupClassName)}>
          ...
          <SearchField.Input className={cn('text-[16px]', inputClassName)} placeholder={placeholder} />
