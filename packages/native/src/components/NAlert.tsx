...
export const NAlert = React.memo<NAlertProps>(
  ({ title, description, message, onClose, className, titleClassName, descriptionClassName, ...props }) => {
    ...
    return (
      <Alert className={cn('w-full bg-surface px-4 py-3', className)} {...props}>
        ...
        <Alert.Content>
          ...
          {body && <Alert.Description className={cn('text-sm', descriptionClassName)}>{body}</Alert.Description>}
        </Alert.Content>
        {onClose && (
