...
export const NLinkify = React.memo<NLinkifyProps>(
  ({ children, className, numberOfLines, linkStyle, onPress }) => {
    const [accentColor] = useThemeColor(["accent"]);

    const defaultLinkStyle = useMemo<TextStyle>(
        ...
        return parts.map((part, i) => {
          ...
          return (
            <Text
              ...
              onPress={() => handlePress(href, part.value)}
            >
              {part.value}
            </Text>
          );
    ...
    const rendered = Array.isArray(children)
      ? children.map((child, i) => renderNode(child, `root-${i}`))
      : renderNode(children, "root");
