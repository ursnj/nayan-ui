  ...
  ({
    ...
    return (
      <Alert status={statusMapping[type]} className={cn("nyn-alert", className)} {...(rest as any)}>
        ...
        <Alert.Content>
          <Alert.Title className={cn("nyn-alert-title", titleClassName)}>
            {title ?? titleMapping[type]}
          </Alert.Title>
          {description != null && (
            <Alert.Description className={cn("nyn-alert-message", messageClassName)}>
              {description}
            </Alert.Description>
          )}
        </Alert.Content>
        {actions}
        {onClose && (
          <CloseButton aria-label="Close alert" className={cn(closeClassName)} onPress={onClose} />
        )}
      </Alert>
    );
  },
