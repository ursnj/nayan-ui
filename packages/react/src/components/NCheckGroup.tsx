    ...
    return (
      <div className={cn('nyn-check-group-block mb-3', className)}>
        ...
        <CheckboxGroup
          ...
          onChange={onChange}
          aria-label={label}
          className={cn('nyn-check-group', orientation === 'horizontal' ? 'flex flex-row flex-wrap gap-4' : 'flex flex-col gap-2')}>
          {items.map(item => (
            <Checkbox key={item.value} value={item.value} isDisabled={item.disabled} className={cn(itemClassName)}>
              <Checkbox.Content>
                <Checkbox.Control>
                  <Checkbox.Indicator />
                </Checkbox.Control>
