...
export const NTheme = React.memo(
  ({ children, theme = null, onThemeChange, ...mainProps }: NThemeProps) => {
    const finalTheme = useMemo(() => {
      const normalizedTheme = typeof theme === "string" ? theme.toLowerCase() : theme;
      if (normalizedTheme === THEMES.LIGHT || normalizedTheme === THEMES.DARK)
    ...
    return (
      <>
        <main
          ...
          tabIndex={-1}
          {...mainProps}
          className={cn("nyn-theme", mainProps.className)}
        >
          {children}
        </main>
        <Toast.Provider />
