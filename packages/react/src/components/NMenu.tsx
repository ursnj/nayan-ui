...
export const NMenu: React.FC<NMenuProps> = React.memo(
  ({
    ...
    return (
      <Dropdown>
        <Dropdown.Trigger className={cn("nyn-menu-trigger", triggerClassName)}>
          {trigger}
        </Dropdown.Trigger>
        <Dropdown.Popover
          placement={placement}
          className={cn("nyn-menu-popover", menuSizeMapping[size])}
        >
          <Dropdown.Menu className={cn("nyn-menu-content", className)}>
            {title && (
              <>
                <Header
                  className={cn(
                    "px-2.5 pb-1 pt-1.5 text-xs font-semibold uppercase tracking-wider text-muted",
                    titleClassName,
                  )}
                >
                  {title}
