      ...
      <Modal>
        <Modal.Backdrop
          ...
          <Modal.Container size={sizeMapping[size]}>
            <Modal.Dialog
              className={cn('nyn-dialog p-0 border border-border bg-card text-white rounded overflow-hidden', maxWidthMapping[size], className)}>
              <Modal.Header className={cn('nyn-dialog-header px-3 py-2.5 bg-primary text-white', headerClassName)}>
                <span className={cn('nyn-dialog-title text-base font-normal', titleClassName)}>{title}</span>
              </Modal.Header>
              <Modal.Body
                className={cn(
                  'nyn-dialog-content text-text h-[calc(100vh_-_140px)] sm:h-auto sm:min-h-[100px] sm:max-h-[calc(100vh_-_75px)] overflow-y-auto',
                  contentClassName
                )}>
                {children}
              </Modal.Body>
