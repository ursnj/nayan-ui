...
const FONTS = [
  ...
  {
    value: '"Lucida Sans Unicode", "Lucida Grande", "DejaVu Sans", sans-serif',
    label: "Lucida Sans",
  },
  ...
  {
    value: '"Arial Black", "Arial Bold", Gadget, "DejaVu Sans Bold", sans-serif',
    label: "Arial Black",
  },
  {
    value: '"Arial Narrow", "Liberation Sans Narrow", "Nimbus Sans Narrow", sans-serif',
    label: "Arial Narrow",
  },
  ...
  {
    value: 'Garamond, "Apple Garamond", "URW Garamond", "Times New Roman", serif',
    label: "Garamond",
  },
  {
    value: 'Baskerville, "Baskerville Old Face", "Libre Baskerville", Georgia, serif',
    label: "Baskerville",
  },
  ...
  return (
    <aside className="island flex h-full min-h-0 flex-col">
      <header className="flex items-center gap-2 border-b border-border px-3 py-2.5">
        ...
        {selectionCount > 1 && (
          <NBadge color="accent" variant="soft" size="sm" className="text-[10px]">
      ...
      {!clip ? (
        <EmptyState
          ...
          hint="Pick a clip on the timeline to edit it."
        />
      ) : (
        <div className="min-h-0 flex-1 overflow-y-auto">
          ...
          {isMediaClip(clip) && clip.kind === "video" && (
            <ChromaSection clip={clip} patch={patch} />
          )}
...
const ClipHeader = ({ clip }: { clip: Clip }) => (
  <div className="border-b border-border px-3 py-2.5">
    <div className="flex items-center gap-2">
      ...
      <NChip size="sm" variant="secondary" className="shrink-0 capitalize text-[10px]">
        {clip.kind}
      </NChip>
    </div>
    <p className="mt-1 font-mono text-[10px] tabular-nums text-muted">
      {formatTimecode(clip.startUs)} → {formatTimecode(clipEndUs(clip))} ·{" "}
      {(clip.durationUs / US).toFixed(2)}s
    </p>
  </div>
);
...
const BasicsSection = ({ clip, patch }: { clip: Clip; patch: Patch }) => {
  ...
  const setSpeed = (next: number) => {
    ...
    const speed = Math.min(
      SPEED_MAX,
      Math.max(SPEED_MIN, Math.round(next / SPEED_STEP) * SPEED_STEP),
    );
    patch({
      speed,
...
const TransformSection = ({ clip, patch }: { clip: Clip; patch: Patch }) => {
  ...
  const set = (changes: Partial<typeof t>) =>
    patch({ transform: { ...t, ...changes } } as Partial<Clip>);
...
const FilterSection = ({ clip, patch }: { clip: Clip; patch: Patch }) => {
  ...
  const applyPreset = (preset: ColorPreset) => {
    ...
    patch({
      colorAdjust: { ...preset.color },
      filter: { name: preset.name, intensity: 1 },
    } as Partial<Clip>);
  };
  ...
  const setIntensity = (intensity: number) => {
    ...
    if (!preset) return;
    patch({
      colorAdjust: blendColor(preset.color, intensity),
      filter: { name: preset.name, intensity },
    } as Partial<Clip>);
  };
...
const ColorSection = ({ clip, patch }: { clip: Clip; patch: Patch }) => {
  ...
  const set = (changes: Partial<typeof color>) =>
    patch({ colorAdjust: { ...color, ...changes }, filter: null } as Partial<Clip>);
    ...
    <>
      ...
      >
        ...
        <SliderField
          ...
          max={100}
          format={(value) =>
            value === 0 ? "Neutral" : value < 0 ? `${-value} cool` : `${value} warm`
          }
        ...
        <SliderField
          ...
          max={100}
          format={(value) =>
            value === 0 ? "Neutral" : value < 0 ? `${-value} green` : `${value} magenta`
          }
        ...
        {color.splitTone > 0 && (
          <div className="grid grid-cols-2 gap-2">
            <ColorField
              label="Shadows"
              value={color.shadowTint}
              onChange={(shadowTint) => set({ shadowTint })}
            />
            <ColorField
              label="Highlights"
              value={color.highlightTint}
              onChange={(highlightTint) => set({ highlightTint })}
            />
          </div>
        )}
      </Section>
      ...
      <Section
        ...
        onReset={() =>
          patch({
            colorAdjust: { ...color, sharpen: 0, vignette: 0, grain: 0, blur: 0 },
            filter: null,
          } as Partial<Clip>)
        }
      >
...
const CropSection = ({ clip, patch }: { clip: Clip; patch: Patch }) => {
  ...
  const set = (changes: Partial<typeof crop>) =>
    patch({ crop: { ...crop, ...changes } } as Partial<Clip>);
...
const AudioSection = ({ clip, patch }: { clip: MediaClip; patch: Patch }) => (
  <Section title="Audio" icon={<Volume2 className="h-3.5 w-3.5 text-muted" />}>
    ...
    <ToggleChip
      active={clip.muted}
      onClick={() => patch({ muted: !clip.muted } as Partial<Clip>)}
...
const ChromaSection = ({ clip, patch }: { clip: MediaClip; patch: Patch }) => {
  ...
  const set = (changes: Partial<typeof key>) =>
    patch({ chromaKey: { ...key, ...changes } } as Partial<Clip>);

  return (
    <Section title="Green screen" icon={<Wand2 className="h-3.5 w-3.5 text-muted" />}>
      <ToggleChip
        active={key.enabled}
        onClick={() => set({ enabled: !key.enabled })}
        label="Remove the key colour"
  ...
  return (
    <Section title="Text" icon={<TypeIcon className="h-3.5 w-3.5 text-muted" />}>
      ...
      <SelectField
        ...
        options={FONTS}
        onChange={(fontFamily) => patch({ fontFamily } as Partial<Clip>)}
      />
      ...
      <div className="mb-2 flex gap-1.5">
        ...
        <ToggleChip
          active={clip.italic}
          onClick={() => patch({ italic: !clip.italic } as Partial<Clip>)}
          label="Italic"
          className="flex-1"
        >
          Italic
      ...
      <ColorField
        ...
        onChange={(textColor) => patch({ textColor } as Partial<Clip>)}
      />
      <ColorField
        label="Background"
      ...
      {clip.strokeWidth > 0 && (
        <ColorField
          ...
          onChange={(strokeColor) => patch({ strokeColor } as Partial<Clip>)}
        />
      )}
    ...
    <Section title="Transition in">
      <SelectField
        ...
        onChange={(value) =>
          setTransition(
            clip.id,
            value === "none" ? null : (value as TransitionKind),
