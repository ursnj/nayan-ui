  ...
  useEffect(() => {
    ...
    void (async () => {
      const entries = await Promise.all(
        EXPORT_FORMATS.map(
          async (option) =>
            [option.id, await isFormatSupported(option, evenWidth, evenHeight)] as const,
        ),
      );
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-2">
        ...
        <SelectField
          label="Format"
          value={formatId}
          options={EXPORT_FORMATS.filter((option) => supported[option.id] !== false).map(
            (option) => ({
              value: option.id,
      ...
      <div className="flex gap-1.5">
        ...
        <ToggleChip
          ...
          label="Export only the marked in/out range"
          className="flex-1"
        >
      ...
      <dl className="rounded-lg bg-surface-secondary px-3 py-2 text-xs">
        <Row
          label="Output"
          value={
            audioOnly ? `${format.label} · audio only` : `${evenWidth} × ${evenHeight} · ${fps} fps`
          }
        />
        <Row
          label="Duration"
          value={
            creditSeconds > 0
              ? `${outputSeconds.toFixed(1)}s · ${creditSeconds}s credit`
              : `${outputSeconds.toFixed(1)}s`
          }
        />
        {!audioOnly && (
          <Row label="Bitrate" value={`${(bitrate / 1_000_000).toFixed(1)} Mbps ceiling`} />
        )}
        <Row
      ...
      {result && (
        <NAlert
          ...
          actions={
            <NLink
              href="#"
              onPress={() => download(result.blob, result.filename)}
