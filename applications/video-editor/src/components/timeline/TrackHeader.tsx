  ...
  const onResizePointerDown = useCallback(
    (event: React.PointerEvent) => {
      ...
      const move = (moveEvent: PointerEvent) => {
        onUpdate({
          height: clamp(startHeight + (moveEvent.clientY - startY), MIN_ROW_HEIGHT, MAX_ROW_HEIGHT),
        });
      };
      const up = () => {
  ...
  return (
    ...
    >
      <div className="flex h-5 items-center gap-1">
        <Icon
          className={cn("h-3.5 w-3.5 shrink-0", track.hidden ? "text-muted/50" : "text-muted")}
        />
        <NInput
          value={track.name}
      ...
      <div
        ...
        onKeyDown={(event) => {
          ...
          onUpdate({
            height: clamp(track.height + direction * step, MIN_ROW_HEIGHT, MAX_ROW_HEIGHT),
          });
        }}
        onKeyUp={held.end}
        onBlur={held.end}
