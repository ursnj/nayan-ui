...
export const TimeRuler = memo(
  ({ width, pxPerSec, viewportLeft, viewportWidth, onScrub }: TimeRulerProps) => {
    ...
    const firstTick = Math.max(
      0,
      Math.floor((viewportLeft - HEADER_WIDTH - VIRTUALISE_OVERSCAN_PX) / tickSpacing),
    );
    const lastTick = Math.min(
      tickCount - 1,
      Math.ceil(
        (viewportLeft + viewportWidth - HEADER_WIDTH + VIRTUALISE_OVERSCAN_PX) / tickSpacing,
      ),
    );
    ...
    return (
      <div style={{ width }} className="relative">
        ...
        >
          ...
          {Array.from(
            { length: visibleTicks },
            (_, offset) => (firstTick + offset + 0.5) * tickSpacing,
          )
