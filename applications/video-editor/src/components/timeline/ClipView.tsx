    ...
    return (
      <div
        ...
        onKeyDown={(event) => {
          ...
          readEditorState().nudgeSelection(
            (event.key === "ArrowLeft" ? -1 : 1) * Math.round((US / fps) * frames),
          );
        }}
        onKeyUp={held.end}
        onBlur={held.end}
        ...
        <div style={{ height: rowHeight - 8 }} className="relative w-full">
          ...
          >
            ...
            <span
              ...
              className="truncate text-[10px] font-medium text-white drop-shadow"
            >
              {clip.name}
            </span>
            ...
            {isMediaClip(clip) && clip.speed !== 1 && (
              <span className="shrink-0 text-[9px] text-white/80">{clip.speed}×</span>
            )}
            {isMediaClip(clip) && clip.reversed && (
              <span className="shrink-0 text-[9px] text-white/80">REV</span>
            )}
          </div>
        </div>
...
const TrimHandle = ({
  ...
  <div
    ...
    className={cn(
      "absolute inset-y-0 z-20 cursor-ew-resize transition-colors hover:bg-accent/80",
  ...
  return (
    <div className="flex h-full w-full overflow-hidden bg-surface-tertiary">
      {Array.from({ length: tileCount }, (_, index) => {
        ...
        const decoded =
          frames.length > 0
            ? frames[Math.min(frames.length - 1, Math.floor((index * frames.length) / tileCount))]
            : "";
...
const Waveform = ({ clip, width }: { clip: MediaClip; width: number }) => {
  ...
  useEffect(() => {
    ...
    for (let x = 0; x < cssWidth; x += barWidth) {
      ...
      const index = Math.min(
        peaks.length - 1,
        Math.max(0, Math.floor((sourceUs / totalUs) * peaks.length)),
      );
  ...
  return (
    <canvas ref={canvasRef} className="h-full w-full" style={{ background: `${clip.color}22` }} />
  );
};
...
const FadeOverlay = ({
  ...
  return (
    ...
    >
      ...
      {fadeOutPx > 0 && (
        <polygon
          points={`${width},0 ${width - fadeOutPx},0 ${width},100%`}
          fill="rgba(0,0,0,0.6)"
        />
      )}
    </svg>
