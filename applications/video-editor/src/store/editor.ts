...
const cuttableAt = (clip: Clip, selectedClipIds: string[], at: number) => {
  ...
  return (
    clip.startUs < at &&
    clipEndUs(clip) > at &&
    at - clip.startUs >= MIN_CLIP_US &&
    clipEndUs(clip) - at >= MIN_CLIP_US
  );
};

/** The clips a split at `timeUs` would cut. */
export const splitTargetsAt = (
  clips: Clip[],
...
const overlaps = (clip: Clip, startUs: number, endUs: number) =>
  clip.startUs < endUs && clipEndUs(clip) > startUs;
...
const insertTrack = (
  ...
  while (
    ...
    titleTrackIds.has(tracks[index].id)
  )
    index++;
...
const normaliseClip = (clip: Clip): Clip => {
  ...
  return isMediaClip(base)
    ? { ...base, fit: base.fit ?? DEFAULT_FIT, chromaKey: { ...DEFAULT_CHROMA, ...base.chromaKey } }
    : base;
};
...
const expandGroups = (clips: Clip[], ids: string[]): string[] => {
  ...
  const groups = new Set(
    clips.filter((clip) => wanted.has(clip.id) && clip.groupId).map((clip) => clip.groupId),
  );
...
export const useEditor = create<EditorState>((set, get) => {
  ...
  const placeClip = (
    ...
    const preferredUsable =
      ...
      !preferred.locked &&
      !state.clips.some(
        (clip) => clip.trackId === preferred.id && overlaps(clip, startUs, startUs + durationUs),
      );

    let track = preferredUsable
      ? preferred
      : findFreeTrack(tracks, state.clips, kind, startUs, durationUs);
    if (!track) {
  ...
  return {
    ...
    removeAsset: (assetId) => {
      ...
      commit((state) => ({
        ...
        selectedClipIds: state.selectedClipIds.filter(
          (id) => !affected.some((clip) => clip.id === id),
        ),
      }));
      void Promise.all(affected.map((clip) => releaseReader(clip.id))).then(() =>
    ...
    updateSelectedClips: (patch) =>
      commit((state) => {
        ...
        return {
          clips: state.clips.map((clip) =>
            selected.has(clip.id) ? ({ ...clip, ...patch } as Clip) : clip,
          ),
        };
      }),
    ...
    toggleSelectionFlip: (axis) =>
      commit((state) => {
        ...
        const targets = new Set(
          state.clips
            .filter((clip) => selected.has(clip.id) && clip.kind !== "audio")
            .map((clip) => clip.id),
        );
        ...
        return {
          clips: state.clips.map((clip) =>
            targets.has(clip.id)
              ? { ...clip, transform: { ...clip.transform, [key]: next } }
              : clip,
          ),
        };
      }),
    ...
    shiftSelectionTrack: (direction) => {
      ...
      for (const clip of targets) {
        ...
        for (
          ...
          index >= 0 && index < state.tracks.length;
          index += direction
        ) {
      ...
      commit((state) => {
        ...
        if (edge === "start") {
          ...
          const updated: Clip = {
            ...
            ...(isMediaClip(clip) && clip.kind !== "image"
              ? { inUs: Math.max(0, clip.inUs + deltaUs * clip.speed) }
              : {}),
          } as Clip;
        ...
        const newEnd = clamp(
          Math.round(timeUs),
          clip.startUs + MIN_CLIP_US,
          clip.startUs + maxDurationUs,
        );
    ...
    paste: (atUs) => {
      ...
      const pasted = state.clipboard.map((clip) => {
        ...
        const trackId = trackExists
          ? clip.trackId
          : (state.tracks.find(
              (track) => track.kind === (clip.kind === "audio" ? "audio" : "video"),
            )?.id ?? clip.trackId);
        ...
        return {
          ...
          clips: [
            ...state.clips.map((entry) =>
              entry.id === clipId ? { ...entry, muted: true, groupId } : entry,
            ),
            audio,
          ],
    ...
    groupSelection: () =>
      commit((state) => {
        ...
        return {
          clips: state.clips.map((clip) =>
            state.selectedClipIds.includes(clip.id) ? { ...clip, groupId } : clip,
          ),
        };
      }),
    ...
    ungroupSelection: () =>
      commit((state) => {
        ...
        return {
          clips: state.clips.map((clip) =>
            ids.includes(clip.id) ? { ...clip, groupId: null } : clip,
          ),
        };
      }),
    ...
    setSelection: (clipIds) =>
      set((state) => ({ selectedClipIds: expandGroups(state.clips, clipIds) })),
    ...
    removeTrack: (trackId) => {
      ...
      commit((state) => {
        ...
        return {
          ...
          selectedClipIds: state.selectedClipIds.filter(
            (id) => !doomed.some((clip) => clip.id === id),
          ),
        };
      });
      void Promise.all(doomed.map((clip) => releaseReader(clip.id)));
      ...
      set((state) => {
        ...
        return {
          ...
          selectedClipIds: state.selectedClipIds.filter((id) =>
            previous.clips.some((clip) => clip.id === id),
          ),
        };
      }),
...
export const bundleEntryFor = (assetId: string, name: string) =>
  `media/${assetId}-${name.replace(/[/\\:*?"<>|]+/g, "_")}`;
