...
export const writeBundle = async (project: ProjectFile): Promise<Blob> => {
  const entries = [
    {
      name: MANIFEST,
      data: new Blob([JSON.stringify(project, null, 2)], { type: "application/json" }),
    },
  ];
  ...
  for (const ref of project.assetRefs) {
    ...
    if (!file)
      throw new BundleError(
        `"${ref.name}" is no longer available, so the project cannot be bundled.`,
      );
...
export const readBundle = async (file: Blob): Promise<OpenedBundle> => {
  ...
  if (!manifest)
    throw new BundleError(
      "This file is not a Nayan UI Video Editor project — it has no project.json inside.",
    );
  ...
  if (project.version !== 1 || !Array.isArray(project.clips))
    throw new BundleError("This bundle was written by an unrecognised version.");
